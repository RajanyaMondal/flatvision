...
const Analytics = () => {
  ...
  if (!data || data.totalProperties === 0) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Market Analytics</h1>
          <p className="text-slate-500">Not enough data to display analytics. Run some predictions first.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-slate-900 mb-2">Dataset Analytics</h1>
        <p className="text-slate-500">Insights derived from the ML reference dataset and test evaluations.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card className="h-96">
          <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider mb-6">Price vs Property Size (Lakhs)</h3>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data.priceHistory}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} />
              <XAxis dataKey="size" stroke="#64748b" fontSize={12} tickLine={false} axisLine={false} name="Area (Sqft)" />
        ...
        <Card className="h-96">
          <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider mb-6">Model Test Accuracy: Actual vs Predicted (Rs)</h3>
          <ResponsiveContainer width="100%" height="100%">
            <ScatterChart>
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
              <XAxis dataKey="actual" type="number" stroke="#64748b" fontSize={12} tickLine={false} axisLine={false} name="Actual (Rs)" tickFormatter={(val) => `${val/100000}L`} />
              <YAxis dataKey="predicted" type="number" stroke="#64748b" fontSize={12} tickLine={false} axisLine={false} name="Predicted (Rs)" tickFormatter={(val) => `${val/100000}L`} />
              <Tooltip 
                contentStyle={{ backgroundColor: '#ffffff', borderColor: '#e2e8f0', color: '#0f172a', borderRadius: '8px', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card className="h-80">
          <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider mb-6">Properties by Layout</h3>
          <ResponsiveContainer width="100%" height="100%">
        ...
        <Card className="h-80">
          <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider mb-6">Properties by Facing</h3>
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie
          ...
          <div className="flex flex-wrap justify-center gap-4 mt-4">
            {data.facingDistribution.map((entry, index) => (
              <div key={entry.name} className="flex items-center gap-2 text-xs text-slate-500 font-medium">
